...
def confirmPoint():
    ...
    poco("pro.huobi:id/points_notice_box_text_number").click()
    if poco(text="View").wait(timeout=10).exists():
        poco(text="View").click()
    else:
        poco.click([0.06222222222222222, 0.065])
        return
    poco(text="Waiting").wait(timeout=10).click()
    for item in secretjson:
        if item["UID"] in UID:
            secret = item["Secret"]
            totp = pyotp.TOTP(secret)
            Auth = totp.now()
            #poco("pro.huobi:id/tv_points_transfer_order_status").click()
            poco("pro.huobi:id/id_transfer_to_me_bottom_btn_right").click()
